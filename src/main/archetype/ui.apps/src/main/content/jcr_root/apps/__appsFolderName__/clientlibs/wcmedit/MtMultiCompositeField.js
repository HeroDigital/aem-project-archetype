            ...
            items.push({
                ...
                items: [
                    "->",
                    {
                        ...
                        handler:function() {
                            if(config.limit == 0 || list.items.getCount() <= config.limit || config.limit==undefined) {
                                list.addItem();
                            } else {
                                CQ.Ext.Msg.show({
                                    title: 'Limit reached',
                                    msg: 'You are only allowed to add ' + config.limit +
                                        ' items to this field.',
    ...
    addItem: function(name, o) {
            ...
            if (this.maxLimit > 0 && this.items.getCount() > this.maxLimit)
            {
               if(this.errorMessage){
                   CQ.Ext.Msg.show({  title:this.errorMessageTitle, msg:this.errorMessage,  buttons: CQ.Ext.Msg.OK,  icon: CQ.Ext.Msg.WARNING });
             	}else{
...
CQ.Ext.reg("multilinktrack", CQ.form.MtMultiCompositeField);
...
CQ.form.MtMultiCompositeField.Item = CQ.Ext.extend(CQ.Ext.Panel, {
    ...
    constructor: function(config) {
        ...
        if(!config.readOnly) {
            ...
            items.push({
                ...
                handler: function() {
                    ...
                    item.ownerCt.fireEvent("removeditem", item.ownerCt);
                }
            }
        });
       }
        config = CQ.Util.applyDefaults(config, {
            ...
            defaults: {
                bodyStyle: "padding:3px;margin:3px"
            },
            items: items
        });
